classdef Cloth < handle
    ...
    methods
        %% ...structors
        function self = Cloth(brickCount)
            if 0 < nargin
            ...
            for i = 1:self.brickCount
                self.clothModel{i} = self.GetClothModel(['Cloth',num2str(i)]);
                % Not random spawn
                basePose = SE3(eul2rotm([0,0,0]), [0,0,0]);
                self.clothModel{i}.base = basePose;
                
                 % Plot 3D model
                plot3d(self.clothModel{i},0,'workspace',[-1 1 -1 1 -0.5 2],'view',[-30,30],'delay',0,'noarrow','nowrist');
        ...
        function delete(self)
            for index = 1:self.brickCount
                handles = findobj('Tag', self.clothModel{index}.name);
    ...
    methods (Static)
        %% GetBrickModel
        function model = GetClothModel(name)
            if nargin < 1
                name = 'Brick';
            end
            [faceData,vertexData] = plyread('Cloth.ply','tri');
